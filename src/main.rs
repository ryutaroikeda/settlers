...
pub fn get_faces(board: &Board) -> Vec<usize> {
    // Use a kind of offset coordinates for hexagons.
    let hex_rows = board.height / 3;
    let hex_columns = board.width / 6;
...
pub fn make_hexagonal_board(board: &mut Board) {
}
